...
> Last updated: 2026-03-06 (Wolf) — cycle 4

### 1. What was shipped since last check-in?

**This cycle (2026-03-06 cycle 4):**
- **Gitignore cleanup** (commit `c02971c`) — removed 41 tracked generated files: 37 `apps/web-frontend/coverage/` HTML report files, 3 SQLite dev databases (`synapps.db`, `synapps_memory.db`, `apps/orchestrator/synapps.db`), `.coverage` pytest data file. Updated `.gitignore` with 4 new entries (`apps/web-frontend/coverage/`, `apps/web-frontend/test-results/`, `.coverage`, `*.db`). Repo is now 16,112 lines lighter. These had been accumulating since the project started.

**Prior cycle (cycle 3):**
- E2E all 4 happy-path tests fixed (`61832e9`): auth localStorage injection + WebSocket completion mock

**Test counts (unchanged):** 1360 backend + 101 frontend + 4 E2E = **1,465 total passing**
**Commits this cycle:** `c02971c` (gitignore cleanup)

---

### 2. What surprised me?

**The `*.db` gitignore pattern was necessary but not sufficient.** `*.db` ignores new `.db` files, but the 3 SQLite databases were already tracked — gitignore has no effect on tracked files. The two-step process (1: `git rm --cached`, 2: update `.gitignore`) is easy to forget; many developers only do step 2 and wonder why the file keeps appearing. The correct mental model: gitignore controls *future* tracking, `git rm --cached` controls *current* tracking.

**16,112 lines of generated content in the repo history.** The coverage HTML files and databases had been in the repo since early development. They inflated every `git clone`, every `git log --stat`, and every diff. This is pure overhead — the content is regenerated by `npx vitest --coverage` on demand. It persisted this long because it wasn't causing test failures and nobody explicitly looked for it.

**No `.gitignore` in `apps/web-frontend/`.** The root `.gitignore` had `/coverage` (root-level only) but nothing covering `apps/web-frontend/coverage/`. When coverage was generated inside the subdirectory, it wasn't ignored. The file also lacked `test-results/` entirely. A project with multiple app subdirectories likely needs per-directory `.gitignore` files or explicit path patterns in the root one.

**`git status --short` without `| grep -v "^D "` was masking this.** The D (deleted) entries were always there but getting lost in the visual noise of the long git status output. The `| grep -v "^D "` filter used to declutter status actually hid the problem by removing the signal.

---

### 3. Cross-project signals

**Two-step gitignore fix is a universal pattern.** For any ASIF project with accidentally-tracked generated files: (1) `git rm --cached -r <path>` to untrack from index, (2) add path to `.gitignore` to prevent future tracking. Doing only step 2 is a common mistake. The `git rm --cached` step must precede any future commits or the files re-appear. This applies to: coverage HTML, SQLite dev databases, `.coverage`, Playwright artifacts, build outputs.

**Generated artifacts silently bloat repos over time.** Coverage HTML (37 files, ~16K lines), SQLite databases, and test artifacts accumulate without causing test failures. The only detection method is periodic `git ls-files | grep -E "coverage|\.db|test-results"` audits. Worth adding to ASIF project health checks alongside test counts and CI status.

**Root `.gitignore` needs explicit subdirectory paths for monorepos.** A single `/coverage` at root only ignores `<root>/coverage/`. Each app subdirectory with generated output needs its own entry: `apps/web-frontend/coverage/`, `apps/backend/dist/`, etc. ASIF project templates should model this explicitly to prevent per-subdirectory accumulation.
...
1. **ChromaDB Memory upgrade** — N-04 is an in-memory dict. The 2Brain dogfood template uses Memory nodes; a real vector store makes PI-001 credible. Obvious next feature initiative. M effort.
2. **Content engine test isolation** — `test_pipeline_with_empty_summary` fails intermittently due to test-order-dependent singleton state. Add a conftest `autouse` fixture to reset the TemplateRegistry and/or MetricsCollector between tests. S effort, prevents future false alarms from the pre-push hook.
3. **Deployment** — on HOLD per CoS. Ready when Asif scopes it.
4. **monaco-editor/dompurify vuln** — monitor upstream. Not actionable until monaco-editor ships a fix.

Everything else is clean: working tree has no tracked generated files, E2E green, hook enforcing, CI green, 1,465 tests passing.

---

### 5. Blockers / Questions for CoS

**No blockers.** No questions requiring CoS response.

**State summary:** The project is in its cleanest state since v1.0 shipped. Every category of known debt has been addressed this session:
- ✓ Orphaned code committed (marketplace, `86ac222`)
- ✓ Pre-push hook enforcing correctly (grep-for-failed, monorepo-aware)
- ✓ E2E green — 4/4 happy-path passing (`61832e9`)
- ✓ Generated artifacts untracked — 42-file cleanup (`c02971c`)
- ✓ CI GREEN on GitHub Actions
- → Deployment on HOLD per CoS (awaiting Asif)
- → ChromaDB Memory upgrade is the clear next initiative

**Portfolio signal (no response needed):** Suggest ASIF health-check template include a `git ls-files | grep -E "\.db|coverage|test-results"` step. Detects accidentally-tracked generated files before they accumulate across sessions. Trivially scriptable.
