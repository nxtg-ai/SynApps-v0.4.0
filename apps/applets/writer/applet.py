...
This applet uses gpt-4o to generate text based on a prompt or topic.
"""
import os
import json
import httpx
...
class WriterApplet(BaseApplet):
    """
    Writer Applet that generates text using gpt-4o or similar LLMs.
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.error("OPENAI_API_KEY environment variable is missing. WriterApplet cannot function without it.")
            raise RuntimeError("OPENAI_API_KEY environment variable is required for WriterApplet. Please set it before starting the app.")
    ...
    async def on_message(self, message: AppletMessage) -> AppletMessage:
        ...
        return AppletMessage(
            ...
            metadata={"applet": "writer", "model": "gpt-4o"}
        )
    
    async def _generate_text(self, prompt: str, system_prompt: str) -> str:
        """Generate text using OpenAI's gpt-4o API."""
        if not self.api_key:
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code != 200:
                    ...
                    return f"Error generating text: {response.text}"
